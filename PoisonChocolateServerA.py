...
Names: Zavier Chambers, Aiden Lehner, Timi Agbale 
Mapping Of Opcodes
   [100]              # QUEUE
   [101]              # LEAVE
...
def relay_room(a, b):
    """ THIS IS WHERE MY DATA IS SENT <--- I am sending "newline terminated" 
    lists exactly how you give them to the server between two sockets."""
    buffers = {a: b"", b: b""}    
    try:
        sockets = [a, b]
        while True:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as ls:
        ...
        def matchmaker():
            while True:
                ...
                logging.info(f"Matched clients into room {room_id} (roles 0 and 1)")
